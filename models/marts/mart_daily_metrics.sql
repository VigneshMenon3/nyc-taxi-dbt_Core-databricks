{{
    config(
        ...
        unique_key='trip_date_vendor_key',
...
with base as (
    ...
    {% if is_incremental() %}
        -- on incremental runs, only process data newer than what we already have
...
daily as (
    select
        ...
        concat(
            cast(date_trunc('day', pickup_datetime) as string),
            '_',
            cast(vendor_id as string)
        )                                                           as trip_date_vendor_key,
